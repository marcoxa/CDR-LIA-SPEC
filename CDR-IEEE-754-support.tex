...
simple syntax:
\begin{alltt}
  (trap-math (&key \textit{before} \textit{after}) <\textit{expr}> <\textit{handler}>* )
\end{alltt}
...
\DDictionaryItem{Function \index{\code{make-float}}}
...
\DDictionaryItem{Functions
  \index{\code{make-short-float}},
  \index{\code{make-single-float}},\\
  \index{\code{make-double-float}},
  \index{\code{make-long-float}}}
...
\DDictionaryItem{Values \index{\code{NAN}}}
...
\DDictionaryItem{Functions \index{\code{is-nan}}, \index{\code{nanp}},
  \index{\code{is-quiet-nan}},
  \index{\code{quiet-nan-p}},\\\index{\code{is-signalling-nan}}, 
  \index{\code{signalling-nan-p}}}
...
\DDictionaryItem{Constant Variables\\
  \index{\code{long-float-positive-infinity}},
  \index{\code{long-float-negative-infinity}},\\
  \index{\code{double-float-positive-infinity}},
  \index{\code{double-float-negative-infinity}},\\
  \index{\code{single-float-positive-infinity}},
  \index{\code{single-float-negative-infinity}},\\
  \index{\code{short-float-positive-infinity}},
  \index{\code{short-float-negative-infinity}},\\
  \index{\code{infL}}
  \index{\code{infD}}
  \index{\code{infF}}
  \index{\code{infS}}  
...
\DDictionaryItem{Type \index{\code{fpe-exception}}}
...
\DDictionaryItem{Type \index{\code{fpe-exceptions-set}}}
...
\DDictionaryItem{Functions \index{\code{supported-exception-p}},\\
  \index{\code{all-supported-exceptions-p}},
  \index{\code{some-supported-exceptions-p}}}
...
\varname{excps} -- A list of \index{\code{fpe-exception}} items.\\
\varname{excp-set} -- An object of type \index{\code{fpe-exception-set}}.
...
environment) and returns an object of type \index{\code{fpe-exception-set}}
with the corresponding flag set.

\DExamples{}

The following example (adapted from \cite{2018:C18}) shows how a piece of
code may decide how to \index{\code{signal}} either
