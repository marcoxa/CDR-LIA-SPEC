...
\title{
\LARGE{\bfseries Support for (a subset of) IEEE~745 in \CL{}}}
...
\emph{signalling} \code{NAN} (see~\cite{IEEE-754}).
...
\section{Floating Point IEEE~754 Respecting Operations}
...
\subsection{Non-computational \CL{} Enviroment IEEE-754 Queries}
...
in \cite{IEEE-754} and Section~12.1.4.3 of \cite{ANSIHyperSpec}.
...
specified with respect to \cite{ANSIHyperSpec} in order to adhere to
the requirements of IEEE~754.  The descriptions and the references for
...
Section~12.1.4 of \cite{ANSIHyperSpec}) assume the behavior of the
underlying IEEE~754 specification \cite{IEEE-754}.  It is assumed that
...
according to the IEEE~754 operations described in Section~5.4.1 of
\cite{IEEE-754}, which is assumed to be the usual behavior specified
for \CL{} by \cite{ANSIHyperSpec} when \varname{a} and \varname{b} are
not \emph{NaN}s or \emph{infinities}.

\noindent
The operations from \cite{IEEE-754} are:
...
When \code{+}, or \code{*} is called with either \varname{a} or \varname{b} being a
...
\noindent
The operation from \cite{IEEE-754} is:
...
non-\clieeeterm{infinities} (see \cite{ANSIHyperSpec}).
