...
subset of features\footnote{We speak of the sin and not of the sinner;
...
\item Functions and macros to handle with the underlying
  \emph{floating point environment} and the interplay between IEEE~754
...
\cite{2008:IEEE-754}, \cite{2018:C18}, and the documentation of
...
\subsubsection{Interaction Between the ``Low Level'' IEEE~754
  ``Signalling'' Machinery and \CL{} Condition Handling}

The IEEE~754 specification (Section~7 of \cite{2008:IEEE-754}) appears to
imply that the actual handling of ``exceptional'' situations should
follow the steps below.  Given an operation $f(x, \ldots)$.
...
\subsubsection{Floating Point Exception Handling Dictionary}
...
underlying IEEE~754 specification \cite{2008:IEEE-754}.  It is assumed that
