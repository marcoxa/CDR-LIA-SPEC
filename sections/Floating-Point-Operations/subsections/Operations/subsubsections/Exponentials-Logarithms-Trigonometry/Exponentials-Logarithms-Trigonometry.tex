...
from the \CL{} standard (moreover, \code{pi} is a constant).
...
defined:
\begin{itemize}
\item \textit{\code{function}}\code{.<} computes the function with rounding downward,
\item \textit{\code{function}}\code{.>} computes the function with rounding upward,
\item \textit{\code{function}}\code{.<>} computes the function with rounding to nearest,
\item \textit{\code{function}} computes the function with the current rounding mode.
\end{itemize}
