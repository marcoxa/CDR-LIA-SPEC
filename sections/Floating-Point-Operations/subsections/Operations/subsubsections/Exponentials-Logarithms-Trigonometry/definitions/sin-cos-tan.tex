...
\item If \varname{x} is not $-0.0$ and \varname{y} is a
  \code{positive-infinity} or a \code{negative-infinity} returns\\
  \code{(complex (* (tan }\varname{x}\code{) 0) (signum }\varname{y}\code{))}
\end{itemize}
...
\item If \code{tan} is called with the real part of
  \varname{radians} being an (approximation of) $\pi/2 + k\pi$, then the\\
  \clname{cl:floating-point-invalid-operation} error is signaled.
