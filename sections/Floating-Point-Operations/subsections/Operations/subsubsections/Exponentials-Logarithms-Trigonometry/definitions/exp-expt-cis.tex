...
The function \code{exp} computes the natural exponentiation of a number. In
...
\item If \varname{x} is not $-0.0$ and \varname{y} is
  $-0.0$ returns \code{(conjugate (exp (complex }\varname{x} \code{0)))}
...
\item If \varname{x} is \code{positive-infinity} and
  ...
  returns \code{(complex (* positive-infinity (cos }\varname{y}\code{)) (*
    positive-infinity (sin}\varname{y}\code{)))}
