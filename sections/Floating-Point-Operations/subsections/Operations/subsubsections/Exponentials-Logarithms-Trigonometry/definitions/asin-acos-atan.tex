...
\item When \varname{n} is a complex number, \varname{x} and \varname{y}
  ...
  \begin{itemize}
  \item If \varname{x} is a \code{negative-zero} returns
    \code{(- (asin (complex 0 (- }\varname{y}\code{))))}
  \item If \varname{y} is a \code{negative-zero} and \varname{x} is not a
    \code{negative-zero} returns
    \code{(conjugate (asin (complex} \varname{x}\code{ (- 0)))}
...
\item When \varname{n1} is a complex
  ...
  \item If \varname{x} is a \code{negative-zero} return
    \code{(- (atan (complex 0 (- }\varname{y}\code{))))}
    
  \item If \varname{y} is a \code{negative-zero} and \varname{x} is not a
    \code{negative-zero} return
