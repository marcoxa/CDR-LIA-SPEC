...
\item Decide whether a \clieeeterm{IEEE exception} should be
  ...
  \item Raise the flags to indicate which exception was signaled (and
    handled by default).
  \end{enumerate}
\end{enumerate}
...
\code{cl:division-by-zero})\marginnote{As noted by phoe, we also need to specify when a condition is "serious" or not and when it is signalled by \code{signal} and when it is signalled by \code{error}}
or when the LIA specification implies that
